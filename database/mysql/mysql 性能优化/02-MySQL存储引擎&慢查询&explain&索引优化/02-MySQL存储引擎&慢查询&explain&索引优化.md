...
​	MySQL5.0支持的存储引擎包含 ： InnoDB 、MyISAM 、MEMORY等，其中`InnoDB和BDB提供事务安全`表，其他存储引擎是非事务安全表。
...
​	InnoDB存储引擎是Mysql的默认存储引擎。InnoDB存储引擎提供了具有`提交、回滚、崩溃恢复能力的事务安全`。但是对比MyISAM的存储引擎，InnoDB写处理效率差一些，并且会占用更多的磁盘空间以保留数据和索引。
...
​	`MyISAM 不支持事务、也不支持外键`，其优势是访问的`速度快`，对事务的完整性没有要求或者以SELECT、INSERT为主的应用基本上都可以使用这个引擎来创建表 。
...
- InnoDB : 是Mysql的默认存储引擎，用于事务处理应用程序，支持外键。如果应用对`事务的完整性`有比较高的要求，在并发条件下要求`数据的一致性`，数据操作除了插入和查询意外，还包含很多的更新、删除操作，那么InnoDB存储引擎是比较合适的选择。InnoDB存储引擎除了有效的降低由于删除和更新导致的锁定， 还可以确保事务的完整提交和回滚，对于类似于计费系统或者财务系统等对数据准确性要求比较高的系统，InnoDB是最合适的选择。
...
MySQL 客户端连接成功后，通过 show [session|global] status 命令可以提供服务器状态信息。 可以根据需要加上参数“session”或者“global”来显示 session 级（当前连接）的计结果或者 global 级（自数据库上次启动至今）的统计结果。默认使用参数是“session”。

下面的命令显示了当前 session 中所有统计参数的值：如查看查询，删除，更新等等的频次

```
-- 查看服务器状态
show [global|session] satus;
-- 缺省时
...
- **show processlist**  : 使用show processlist命令查看当前MySQL在进行的线程，包括线程的`状态、是否锁表等`，实时地查看 SQL 执行情况，同时对一些锁表操作进行优化。
...
通过以上步骤查询到效率低的 SQL 语句后，可以通过 EXPLAIN或者 DESC命令获取 MySQL如何执行 SELECT 语句的信息。

explain并不会去真正执行sql语句，但是如何存在中间查询还是会执行的。
...
| 字段          | 含义                                                         |
| ------------- | ------------------------------------------------------------ |
| id            | select查询的序列号，表示查询中执行select子句和select操作顺序。值越大则优先执行。 |
| select_type   | 表示 `SELECT 的类型`，常见取值： SIMPLE（简单表，即无表连接或者查询）、PRIMARY（主查询，主select）、UNION（UNION 中的第二个或者后面的查询语句）、SUBQUERY（ SELECT中的子查询），派生（FROM中的select查询）等 |
| table         | select查询的表或中间表                                       |
| type          | 表示表的`连接类型`，性能由好到差的连接类型： system  -->  const  -->  eq_ref  -->  ref  -->  ref_or_null -->  index_merge  -->  index_subquery -->  range  -->  index  --> all 。最次也要range以上 |
...
`id 字段`是 select查询的序列号，表示的是查询中执行select子句或者是操作表的顺序。
...
| PRIMARY      | 查询中若包含任何复杂的select子查询，最外层查询标记为该标识   |
| SUBQUERY     | 在SELECT 或 WHERE 列表中包含select子查询                     |
| DERIVED      | 在FROM 列表中包含的select子查询，被标记为 DERIVED（衍生） MYSQL会递归执行这些子查询，把结果放在临时表中 |
...
type 显示的是**访问类型**（关联类型），是较为重要的一个指标，可取值为： 

| type   | 含义                                                         |
| ------ | ------------------------------------------------------------ |
| NULL   | 不访问任何表，索引，直接返回结果                             |
| system | 表只有一行记录(等于系统表)，这是const类型的特例，一般不会出现 |
| const  | `表示通过索引一次就找到`，primary key 或者 unique 索引。因为**只匹配一行数据，所以很快**。const于将 **"主键" 或 "唯一"** 索引的所有部分与常量值进行比较。 |
| eq_ref | 类似ref，区别在于使用唯一索引或主键关联查询。常见于**主键或唯一索引扫描** |
| ref    | **非唯一性索引扫描**，返回匹配某个单独值的所有行（非唯一性索引的查询，不一定是关联查询）。 |
| range  | 带索引的范围查询。 where 之后出现 between ， < , > , in 等操作。 |
| index  | 扫描所有索引， 通常比ALL 快， ALL 是遍历数据文件。           |
| all    | 全表扫描                                                     |
...
<font color = "red">  一般来说， 我们需要保证查询至少达到 range 级别， 最好达到ref 。</font>
...
| extra                     | 含义                                                        |
| ------------------------- | ----------------------------------------------------------- |
| `Using index`             | 使用覆盖索引，不读数据文件，只从索引文件获取数据。          |
| `Using where`             |                                                             |
...
<font color="red">using filesort 和 using temporary 出现了非常消耗性能，要考虑优化了。如果出现了，创建索引</font>

![image-20220426231821083](assets/image-20220426231821083.png)

****

### 3.10 show profile 分析SQL

Mysql从高版本增加了对 show profiles 和 show profile 语句的支持。show profiles 能够在做SQL优化时帮助我们了解**时间都耗费到哪里去了**。

通过 have_profiling 参数（系统变量），能够看到当前MySQL是否支持profile：
...
**了解即可： ** 在获取到最消耗时间的线程状态后，MySQL支持进一步选择all、cpu、block io 、context switch、page faults等明细类型类查看MySQL在使用什么资源上耗费了过高的时间。例如，选择查看CPU的耗费时间  ：
...
MySQ提供了对SQL的跟踪trace, 通过trace文件能够进一步了解为什么优化器选择A计划, 而不是选择B计划。
...
在我们准备的表结构tb_item 中， 一共存储了 300 万记录；（不用索引4.5s）
...
处理方案 ， 针对title字段， 创建索引 （普通索引）： create index 。。。 on 表（字段）
...
--- 创建联合索引或者使用alter table xxx add index index—name(...)
...
`全值匹配 ，对索引中所有列都指定具体值（索引有效（联合索引下））。`

改顺序情况下，索引任然生效，这是由于MySQL优化器自动调整顺序进行优化的结果。
...
>   即联合索引是有序的多个索引在一起相当于整数的个，百，千位一样，如果位数的顺序乱了或者缺失了是无法比较大小的。仅高位即可判断大小的底位可以缺失，顺序上mysql是存在优化的。
...
**注意：由于有优化器的存在，满足最左的字段即使顺序错乱了索引任存在**

#### 3). 范围查询之避免索引失效

如果范围查询时，返回右边的列，则左边的列不要使用范围查询，否则范围条件之后的字段索引将失效。

**范围查询**：!=或 <= 或 >= 或 betwent 或 in（IN-可能索引存在）等等

![1556172256791](assets/1556172256791.png) 

**原因**：

>   还是因为索引是有序的数据结构，左边高位是范围，是模糊的，右边是低位其大小无法决定谁排前谁排后。
>
>   如整数：191和159 个位决定不了大小。

#### 4). 索引列运算操作之索引失效

>   索引是有序的数据结构，对索引列进行运算相当于索引列都运算完成之后得到新的结果表在进行查询，运算之后的表若还根据原字段排序肯定和原来不同。
>
...
#### 6). 尽量使用覆盖索引
 ...
 尽量使用覆盖索引（索引覆盖：只访问(返回有)索引的查询（索引列完全包含查询列）），减少select * 。

如果没有索引，则通过普索引查找主键，在通过主键查找该列，出现了回表现象。

*   using index：使用索引覆盖 

![image-20220426223748552](assets/image-20220426223748552.png)

如果查询列，超出索引列，也会降低性能（出现回表）。

![image-20220426223853854](assets/image-20220426223853854.png) 
...
**回表查询** ：select获取数据的时候还是需要到表结构文件中进行查询（从索引当中拿到主键，然后根据主键去表结构当中去拿到对应数据一整行的数据，索引当中没有记录所有的数据只是记录这个字段的数据）
...
原因在于索引是有序的数据结构，or条件无法比较大小，如： 10 or 100 与 1 or 1000，就没法通过排序去查找。

#### 8).  like查询之使用索引

以%开头的Like模糊查询，索引失效，以%结尾的模糊查询索引任然有效。

如果仅仅是尾部模糊匹配，索引不会失效。如果是头部模糊，索引失效。
...
1.   **如果MySQL评估使用索引比全表更慢，则不使用索引。（和表的数据有关，所以建立了索引，索引不一定提高效率）**

![1556175445210](assets/1556175445210.png) 

****

2.   **is  NULL ， is NOT NULL  <font color='red'>有时</font>索引失效。（和数据有关，mysql自己判断走全表还是走索引）**

![1556180634889](assets/1556180634889.png)  

****

**3. in 走索引， not in 索引失效。（不要使用not in）**
...
​		尽量使用复合索引（相当创建了多个索引），而少使用单列索引（多个单列索引都用到时，只有一个生效） 。
...
​		当进行数据的insert操作的时候，可以考虑采用以下几种优化方案（原理：1.主键有序 。2.批量插入）。
...
>    		order by操作就是排序，MySQL支持两种方式的排序filesort文件排序和index索引排序，Using index是指MySQL扫描索引本身就完成排序。如此，index 排序效率高，filesort排序效率低。
>
>   **索引排序条件**：
>
>   *    order by语句使用索引的前导列
>   *    使用where条件与order by条件的feild组合在一起满足索引最左前列原则
>
>   **文件排序条件**：
>
>   1.   如果order by的条件无索引列
>   2.   group by与order by很类似，其实质是`先排序后分组`，遵照索引创建顺序的最左前缀法则。
>
>   ps：**对于group by的优化如果不需要排序的可以加上order by null禁止排序。**
...
​		所有不是通过索引直接返回排序结果的排序都都会产生 FileSort 排序。
...
ps：where 条件和Order by 使用相同的索引，并且Order By 的顺序和索引顺序相同， 并且Order  by 的字段都是升序，或者都是降序，否则出现FileSort。
...
​			由于GROUP BY 实际上也同样会进行排序操作，而且与ORDER BY 相比，GROUP BY 主要只是多了排序之后的分组操作。所以，在GROUP BY 的实现过程中，与 ORDER BY 一样也可以利用到索引。
...
​		Mysql支持SQL的子查询。这个技术可以使用SELECT语句来创建一个单列的查询结果，然后把这个结果作为过滤条件用在另一个查询中。使用子查询可以一次性的完成很多逻辑上需要多个步骤才能完成的SQL操作，同时也可以避免事务或者表锁死，并且写起来也很容易。但是，有些情况下，子查询是可以被更高效的连接替代。
...
​		<font color="red">连接查询之所以更有效率一些 ，是因为不需要在内存中创建临时表来完成这个逻辑上需要两个步骤的查询工作。</font>
