...
* schema：逻辑库（看作真实一台物理数据库服务器中一个数据库理解，分库）

* tableA/B：逻辑表（分表的汇总）

* tableA：考虑到拆分（分表）将tableA中的数据分散存在两个数据节点中DataNode1/2

* tableB：考虑到拆分（分表将tableA中的数据分散存在两个数据节点中DataNode3/4

* DataNodex：（分片）数据节点的数据存储在指定物理数据库中 

  连接mycat中一个逻辑库，就可以操作旗下多台物理数据库
...
关系型数据库是基于实体关系模型(Entity Relationship Model)的, MyCat中的ER表便来源于此。 MyCat提出了基于ER关系的数据分片策略 , 字表的记录与其所关联的父表的记录存放在同一个数据分片中, 通过表分组(Table Group)保证数据关联查询不会跨库操作。（order关联user，将这两张表的管理的数据存放同一个节点中从而避免跨库操作）
...
schema.xml 作为MyCat中重要的配置文件之一，管理着MyCat的**逻辑库**、**逻辑表**以及对应的**分片规则**、**DataNode**以及**DataSource**。弄懂这些配置，是正确使用MyCat的前提。这里就一层层对该文件进行解析。
...
<mycat:schema xmlns:mycat="http://io.mycat/">
	...
	<schema name="ITCAST" checkSQLschema="false" sqlMaxLimit="100">
        <!-- table：逻辑表配置
			name：逻辑表的表名
			dataNode：（分片）这个逻辑表在哪些数据节点之中存在，对应下面dataNode的定义
			rule：分片规则
 		-->
		<table name="TB_TEST" dataNode="dn1,dn2,dn3" rule="auto-sharding-long" />
	</schema>
    
    <!-- dataNode：数据节点配置 ，每个dataNode关联一个真实的物理数据库
		name：数据节点名
		dataHost: 物理主机
		database: 关联数据库服务器中哪个数据库，一个应用一般一个数据库
	-->
	...
	<dataHost name="host1" maxCon="1000" minCon="10" balance="0"
        ...
        <!--
			heartbeat:mycat当中的心跳：监测下面的mysql是否还存活
		-->
...
server.xml几乎保存了所有mycat需要的系统配置信息。最常用的是在此配置mycat**用户名**、**密码**及**权限**。在system中添加UTF-8字符集设置，否则存储中文会出现问号
...
修改user的设置 ,  我们这里为 ITCAST 设置了两个用户 : （**是mycat自己的用户密码，非MySQL的，Java连接的是mycat**）

```xml
<!--
	<user name="root": mycat的用户名root
	<property name="password">123456：mycat用户密码
	<property name="schemas">ITCAST：这个用户能操作的逻辑库
	<property name="readOnly">true: 这个用户只读
-->
<user name="root">
...
* 连接mycat，mycat就相当一个伪造的mysql，有库有表不过时逻辑的
